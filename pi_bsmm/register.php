...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt_cek = $pdo->prepare("SELECT id FROM customers WHERE email = ?");
    $stmt_cek->execute([$email]);
    
    if($stmt_cek->rowCount() > 0) {
        $error = "Email tersebut sudah terdaftar! Silakan gunakan email lain atau Login.";
    } else {
        $stmt_insert = $pdo->prepare("INSERT INTO customers (nama_lengkap, email, password, no_telp, alamat_lengkap) VALUES (?, ?, ?, ?, ?)");
        
        if($stmt_insert->execute([$nama, $email, $password, $telp, $alamat])) {
